...
Sudoku::Sudoku(std::vector<int> vector) {
    assert(vector.size() == 81);
    std::vector<std::vector<int>> constructed_grid = std::vector<std::vector<int>>();

    for (int i = 0, count = 0; i < 9; i++) {
        constructed_grid.emplace_back();
        for (int j = 0; j < 9; j++, count++) {
            constructed_grid[i].push_back(vector[count]);
        }
    }

    this->grid = std::move(constructed_grid);
    set_bitsets();
    set_input_mask();
}
